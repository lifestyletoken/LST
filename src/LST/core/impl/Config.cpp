...
#include <skywell/core/ConfigSections.h>
#include <skywell/basics/Log.h>
#include <skywell/json/json_reader.h>
#include <skywell/protocol/SystemParameters.h>
#include <skywell/net/HTTPClient.h>
...
// Fees are in SWT.
#define DEFAULT_FEE_DEFAULT             1000000
#define DEFAULT_FEE_ACCOUNT_RESERVE     10*SYSTEM_CURRENCY_PARTS
#define DEFAULT_FEE_OWNER_RESERVE       5*SYSTEM_CURRENCY_PARTS
#define DEFAULT_FEE_OFFER               DEFAULT_FEE_DEFAULT
#define DEFAULT_FEE_OPERATION           1000000

// Fee in fee units
#define DEFAULT_TRANSACTION_FEE_BASE    1000000
...
void Config::loadFromString (std::string const& fileContents)
{
    ...
    if (getSingleSection (secConfig, SECTION_FEE_ACCOUNT_RESERVE, strTemp))
        FEE_ACCOUNT_RESERVE = beast::lexicalCastThrow <std::uint32_t> (strTemp);

    if (getSingleSection (secConfig, SECTION_FEE_OWNER_RESERVE, strTemp))
        FEE_OWNER_RESERVE   = beast::lexicalCastThrow <std::uint32_t> (strTemp);
