...
#include <skywell/core/LoadMonitor.h>
...
void LoadMonitor::updateData(std::string const& jobName,double const& runTime)
{
  ...
  else
  {
    ...
    get<0>(value) += 1; 
    double maxRunTime = get<1>(value);
    get<1>(value) = (maxRunTime > mSeconds) ? maxRunTime : mSeconds ;
    get<2>(value) += mSeconds;
  ...
  auto func = [&jvArray,&mSecondsFunc](pair_type const& ib)  
        {
          ...
          jTmp["job_times"] = get<0>(ib.second);
          jTmp["max_runTime"] = mSecondsFunc(get<1>(ib.second));
          jTmp["total_runTime"] = mSecondsFunc(get<2>(ib.second));
          jTmp["avg_runTime"] =(get<0>(ib.second) == 0)?"0.0": mSecondsFunc(get<2>(ib.second) / get<0>(ib.second));
          jvArray.append(jTmp);
        };
