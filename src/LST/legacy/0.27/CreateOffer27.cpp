...
#include <skywell/legacy/0.27/book/OfferStream.h>
#include <skywell/legacy/0.27/book/Taker.h>
#include <skywell/legacy/0.27/book/Types.h>
#include <skywell/legacy/0.27/book/Amounts.h>
#include <skywell/legacy/0.27/CreateOffer.h>
...
CreateOffer::checkAcceptAsset(IssueRef issue) const
{
    ...
    if (issuerAccount->getFieldU32 (sfFlags) & lsfRequireAuth)
    {
        SLE::pointer const trustLine (mEngine->view().entryCache (
            ltSKYWELL_STATE, getSkywellStateIndex (
...
CreateOffer::doApply()
{
    ...
    {
        m_journal.warning <<
            "Malformed offer: "<<getConfig().NAME<<" for "<<getConfig().NAME;
    ...
    // We don't allow a non-native currency to use the currency code SWT.
