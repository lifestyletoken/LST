...
Json::Value doBookOffers (RPC::Context& context)
{
    ...
    std::string code1 = "Unneeded field 'taker_pays.issuer' for ";
                code1 += getConfig().NAME;
                code1 += " currency specification.";
    if (isSWT (pay_currency) && ! isSWT (pay_issuer))
        return RPC::make_error (
            rpcSRC_ISR_MALFORMED, code1);

    std::string code2 = "Invalid field 'taker_pays.issuer', expected non-";
                code2 += getConfig().NAME;
                code2 += " issuer. ";  
    if (!isSWT (pay_currency) && isSWT (pay_issuer))
        return RPC::make_error (rpcSRC_ISR_MALFORMED,
            code1);
    ...
    std::string code = "Unneeded field 'taker_gets.issuer' for ";
                code += getConfig().NAME;
                code += " currency specification.";   
    if (isSWT (get_currency) && ! isSWT (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            code);

  
    std::string code3 = "Invalid field 'taker_gets.issuer', expected non-";
                code3 += getConfig().NAME;
                code3 += " issuer."; 
    if (!isSWT (get_currency) && isSWT (get_issuer))
        return RPC::make_error (rpcDST_ISR_MALFORMED,
            code3);
